...
$id = $_GET['updateid'];
$sql2 = "SELECT * FROM assigment_table WHERE id='$id'";
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE assigment_table SET fname='$fname',lname='$lname',email='$email',password='$password',user='$user' WHERE id='$id'";
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
                            <div class="row gy-3 gy-md-4 overflow-hidden">
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control" name="firstName" id="firstName" placeholder="First Name" value="<?php echo $fname; ?>"><?php echo $fnameERR; ?>
                                </div>
                                <div class="col-12">
                                    <label for="lastName" class="form-label">Last Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="lastName" id="lastName" placeholder="Last Name" value="<?php echo $lname; ?>"><?php echo $lnameERR; ?>
                                </div>
                                <div class="col-12">
                                    <label for="email" class="form-label">Email <span class="text-danger">*</span></label>
                                    <input type="email" class="form-control" name="email" id="email" placeholder="name@example.com" value="<?php echo $email; ?>"><?php echo $emailERR; ?>
                                </div>
                                <div class="col-12">
                                    <label for="password" class="form-label">Password <span class="text-danger">*</span></label>
                                    <input type="password" class="form-control" name="password" id="password" value="<?php echo $password; ?>"><?php echo $passwordERR; ?>
                                </div>
                                <!-- Default radio -->
                                <div class="form-check">
                                    <input class="form-check-input" <?php if ($user == "on") {
                                                                        echo "checked";
                                                                    } ?> type="radio" name="user" value="on" id="flexRadioDefault1" />
                                    <label class="form-check-label" for="flexRadioDefault1"> Admin </label>
                                </div>

                                <!-- Default checked radio -->
                                <div class="form-check">
                                    <input class="form-check-input" <?php if ($user == "off") {
                                                                        echo "checked";
                                                                    } ?> type="radio" name="user" value="off" id="flexRadioDefault2" />
                                    <label class="form-check-label" for="flexRadioDefault2"> Normal User </label>
                                </div>
                                <?php echo $userERR; ?>
                                <div class="col-12">
                                    <div class="d-grid">
                                        <button class="btn bsb-btn-xl btn-primary" name="submit" type="submit">Update</button>
                                    </div>
                                </div>
                            </div>
                        </form>
